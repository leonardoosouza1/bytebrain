...
log(f"\n{'='*72}\n# WS10b — ÓRGÃO-BYTE v2.1 (escalado: h384, 9k passos, 35% identidade) (denoiser aprendido, held-out estrito) — {time.strftime('%H:%M')}\n{'='*72}")
t0=time.time()
...
for step in range(STEPS):
    ...
    for _ in range(B):
        ...
        src = w if r.random()<0.35 else typo(w,r.randrange(4),r)
        t=enc(w)
    ...
    if (step+1)%1800==0: log(f"  passo {step+1}/{STEPS} · loss {loss.item():.3f} · {time.time()-t0:.0f}s")
...
torch.save(model.state_dict(), os.path.join(HERE,"byte_denoiser_v21.pt"))
log("pesos salvos: bytebrain/research/byte_denoiser_v21.pt")
