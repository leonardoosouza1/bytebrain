...
CORPUS=cA+cB
log(f"  corpus UNIÃO colhido: {len(CORPUS)} sentenças (A={len(cA)} + B={len(cB)})")
...
json.dump(dict(base=b0,refined=r,corpus=len(CORPUS)),open(os.path.join(HERE,"ws14_refino.json"),"w"),indent=1)
