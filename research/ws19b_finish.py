...
# ============ FASE 1b: completar o grafo (7B faultou aos 120/180) com o 3B confiável ============
GRAPH={}; SRC={}
d=json.load(open(CKPT)); GRAPH={tuple(k.split("|")):v for k,v in d["graph"].items()}; SRC={tuple(k.split("|")):s for k,s in d.get("src",{}).items()}
log(f"FASE 1: {len(GRAPH)} arestas PREMIUM do 7B (checkpoint; 7B GPU-faultou aos 120/180 — honesto)")
PROMPT3={"capital":["The capital of {} is","The capital city of {} is"],"continent":["The continent where {} is is","{} is on the continent of"],
 "region":["Which world region is {} in? Answer:","{} is in the region of"],"language":["The main language of {} is","People in {} speak"],
 "currency":["The currency of {} is the","In {} people pay with the"],"hemisphere":["Is {} Northern or Southern hemisphere? Answer:","{} is in the hemisphere:"]}
missing=[(c,r) for c in COUNTRIES for r in RELS if (c,r) not in GRAPH]
if missing:
    log(f"  completando {len(missing)} arestas com o 3B (GPU, multi-prompt)...")
    ...
    @torch.no_grad()
    def gen3(p,n=10):
        ids=tok3(p,return_tensors="pt").input_ids.to(DEV)
        o=m3.generate(ids,max_new_tokens=n,do_sample=False,pad_token_id=tok3.eos_token_id)
        return tok3.decode(o[0,ids.shape[1]:],skip_special_tokens=True).split("\n")[0].strip()
    for c,r in missing:
        for pr in PROMPT3[r]:
            v=match_canon(r,gen3(pr.format(c)))
            if v: GRAPH[(c,r)]=v; SRC[(c,r)]="3B"; break
...
prov={s:sum(1 for x in SRC.values() if x==s) for s in set(SRC.values())}
log(f"  grafo final: {len(GRAPH)} arestas · proveniência {prov}")
cur=sum(1 for (c,r),v in GRAPH.items() if has(v,KB[c][r]) or has(KB[c][r],v))
log(f"  curadoria: {cur}/{len(GRAPH)} = {cur/max(1,len(GRAPH)):.0%} corretas")
INV={}
